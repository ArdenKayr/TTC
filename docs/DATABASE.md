...
В базе **13 таблиц**. Деньги (сборы, подписки, транзакции) в бете не нужны — их таблицы удалены
...
| `needs_text` | Что нужно, чтобы мероприятие состоялось: люди, деньги, помещение, реквизит — свободным текстом. Пусто только у заявок, поданных до появления этого шага |
...
а автор с ролью «пользователь» автоматически становится «организатором». Когда админ в разделе
«📅 Мероприятия» помечает мероприятие завершённым или отменённым — карточка в Афише правится,
...
| `title`, `description`, `needs_text`, `organizers_text`, `plan_url`, `chat_url` | Скопированы из заявки |
...
Участник нажимает «📊 Предложить голосование», пишет вопрос и варианты ответа, затем выбирает
тип опроса. После «Принять» бот публикует настоящий Telegram-опрос в топик «Голосования».
...
| `is_anonymous` | «да» — видны только цифры; «нет» — видно, кто как проголосовал. Выбирает автор в анкете; после публикации Telegram сменить это не даёт (только пересоздать опрос). У заявок, поданных до появления выбора, — «да» |
| `status` | `pending` / `approved` / `rejected` |
| `processed_by`, `processed_at` | Какой админ и когда решил |
...
В списке таблиц будет ещё четырнадцатая — `alembic_version`: это служебная запись «до какой миграции
