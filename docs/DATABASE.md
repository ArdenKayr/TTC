...
| `bot_settings` | Переключатели поведения бота, которые владелец меняет из панели (автоприём заявок) |
...
| `photo_file_id` | Обложка поста в Афише — номер фотографии в хранилище Telegram. Пусто только у заявок, поданных до появления этого шага |
...
| `title`, `description`, `photo_file_id`, `needs_text`, `organizers_text`, `plan_url`, `chat_url` | Скопированы из заявки |
...
| `afisha_is_caption` | Карточка ушла подписью под картинкой (иначе — отдельным текстом под ней: в подпись Telegram пускает только 1024 символа). От этого зависит, что править при закрытии |
...
### `bot_settings` — переключатели, меняемые из панели
...
редко и требует доступа к серверу. Здесь — то, что владелец решает по ходу
жизни сообщества и может захотеть переключить в любую минуту, в том числе с
телефона: сейчас это единственный ключ `auto_approve` (автоприём заявок).

Таблица бывает пустой. Отсутствие ключа означает «выключено» — так задумано:
обновление, добавившее переключатель, не должно ничего включать само.

| Колонка | Что лежит |
|---|---|
| `key` | Что переключаем («auto_approve»). **Главный ключ** |
| `value` | «on» или «off» |
| `updated_by` | → `users.tg_id` — кто переключил последним |
| `updated_at` | Когда |
...
| `0014` | + картинка к мероприятию: `photo_file_id` в заявке и мероприятии, `afisha_is_caption` — каким получился пост в Афише |
