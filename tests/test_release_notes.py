...
Правила — это шесть договорённостей о том, как выпускается обновление. Здесь
проверяется всё, что вообще проверяется машинально: обновление заводится только
по команде владельца (черновик без номера в сборку не идёт), у выпущенного есть
...
def test_draft_is_not_a_release():
    """Правило 1: пока владелец не сказал «выпускаем», обновления не существует.

    Черновик лежит там же, рядом с выпущенными, и отличается от них ровно
    одним — отсутствием номера. Если сборщик перестанет видеть эту границу, он
    снова начнёт делать статьи, которые никто не просил и никуда не понесёт.
    """
    assert DRAFT.parent == RELEASES, "черновик живёт рядом с обновлениями"
    assert DRAFT not in sources(), (
        "черновик попал в список обновлений — из него соберутся страницы"
    )
    for path in sources():
        assert path.name[:2].isdigit(), (
            f"{path.name} считается обновлением, но у него нет номера"
        )


def test_draft_has_no_number_and_no_name():
    """У черновика нет шапки обновления: номер и название даёт только команда."""
    if not DRAFT.exists():
        pytest.skip("черновика нет — всё выпущено")
    header = DRAFT.read_text(encoding="utf-8").split(USERS_MARKER, 1)[0]
    assert "# Обновление" not in header, (
        "в черновике уже стоит заголовок обновления — его ставят при выпуске"
    )
