...
Обновление заводится только по команде владельца (правило 1). До неё изменения
копятся в черновике docs/releases/next.md — у него нет номера, и сборщик его не
трогает: страницы, о которых не просили, никому не нужны.
...
# Выпущенное обновление — файл с номером. Черновик следующего лежит рядом без
# номера: номер и название ему даёт команда владельца, а до тех пор он ничем не
# отличается от заметок и в статьи не превращается.
NUMBERED = "[0-9][0-9]-*.md"
DRAFT = RELEASES / "next.md"
...
# «# Обновление 1 — «Понятный бот»»: номер и название (правило 1).
...
def sources() -> list[Path]:
    """Выпущенные обновления, по порядку. Черновик без номера сюда не попадает."""
    return sorted(RELEASES.glob(NUMBERED))
...
def main() -> int:
    ...
    if DRAFT.exists():
        print(
            f"\nРядом лежит черновик следующего обновления: {DRAFT.name}.\n"
            "Страницы из него не собираются: номер и название обновление получает\n"
