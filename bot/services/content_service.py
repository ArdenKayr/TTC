...
def formatted_text(message: Message) -> str:
    """Текст (или подпись) присланного админом сообщения ВМЕСТЕ с оформлением.

    Telegram присылает оформление отдельно от букв: сам текст плюс список
    пометок «с 0-го символа по 2-й — премиум-эмодзи №5368…», «с 10-го по 15-й —
    жирный». Если взять только ``message.text``, пометки теряются и в базу
    ложится голый текст: премиум-эмодзи вырождается в обычную, жирный пропадает.

    Здесь пометки переводятся обратно в HTML-теги (``<tg-emoji emoji-id="…">``,
    ``<b>``, ``<a href="…">``) — бот отправляет сообщения в режиме HTML, поэтому
    получатель увидит ровно то, что админ набрал в Telegram. Обычные символы
    при этом экранируются, так что «<» в тексте больше не ломает отправку.

    Обратная сторона: HTML-теги, набранные руками, теперь показываются как
    текст — оформлять надо средствами Telegram, а не тегами.
    """
