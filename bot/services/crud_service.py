...
LINK_LABELS: dict[tuple[str, str], tuple[tuple[str, str, str], str]] = {
    ...
    ("report_messages", "author_tg_id"): (
        ("сообщение по репорту", "сообщения по репорту", "сообщений по репорту"),
        "он написал",
    ),
    # Ссылки на вуз
    ("users", "university_id"): (
    ...
    ("report_messages", "report_id"): (
        ("сообщение переписки", "сообщения переписки", "сообщений переписки"),
        "уйдут вместе с репортом",
    ),
}
